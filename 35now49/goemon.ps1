...
{
$time_n=get-date
$timecheck= ($time_n-(gi -path "D:\Public\_AutoTask\RC\_autoprogram\Goemon_summary_更新中勿操作.csv").LastWriteTime).TotalMinutes
if($timecheck -gt 60){
...
$wshell = New-Object -ComObject wscript.shell
 ...
 remove-item D:\Users\user30\Downloads\* -Force
  ...
  if ((get-process edge -ea SilentlyContinue).HandleCount.count -ne 0){
 taskkill /IM msedge. /T /F
 #wmic process where "name='chrome.exe'" delete
 }

 try{
 Invoke-WebRequest -Uri "https://goemon.necp.co.jp/procenter/jsp/index.jsp" -UseBasicParsing
 }catch{
 write-host "fail to connect  - $_"
 Start-Sleep -s 5
 #exit
 }
  
Start-Process msedge.exe "https://goemon.necp.co.jp/procenter/jsp/index.jsp"
...
start-sleep -s 1
$wshell.SendKeys("^c")
$contenta=get-Clipboard
start-sleep -s 5
$wshell.SendKeys("{ESC}")

IF($contenta -match "パスワードは定期的に変更してください" ){
$wshell.SendKeys("~")
}
...
$check_match= ((import-csv $csv_newfile  -Encoding UTF8 -Delimiter `t | where-object{$_."名前" -notmatch "議事録" -and $_."名前" -notmatch "Q&A" -and $_."名前" -notmatch "環境情報" -and $_."ファイル数" -ne 0}).ID).count
...
import-csv $csv_newfile  -Encoding UTF8 -Delimiter `t | where-object{$_."名前" -notmatch "議事録" -and $_."名前" -notmatch "Q&A" -and $_."名前" -notmatch "環境情報" -and $_."ファイル数" -ne 0}|Export-Csv $csv_newfile2 -NoTypeInformation  -Encoding UTF8
...
$check_match= ((import-csv $csv_newfile  -Encoding UTF8 -Delimiter `t | where-object{$new_BIOS -match $_."名前"}).ID).count
if($check_match -gt 0){
...
import-csv $csv_newfile  -Encoding UTF8 -Delimiter `t | where-object{$new_BIOS -match $_."名前"}|Export-Csv $csv_newfile2 -NoTypeInformation  -Encoding UTF8
$newcsv=import-csv -path $csv_newfile2  -Encoding UTF8
...
$len=($newcsv0."更新日").length-3

#$newID=$newcsv0."ID"+"-"+(($newcsv0."修改日期".Substring(0,$len)).replace("/0","/")).replace(" 0"," ")
$newID=$newcsv0."ID"+"-"+(($newcsv0."更新日".Substring(0,$len)).replace("/0","/")).replace(" 0"," ")
...
$typef=($newcsv|where-Object {$_."ID" -match $dif})."種別"
$name0=($newcsv|where-Object {$_."ID" -match $dif})."名前"
$count=($newcsv|where-Object {$_."ID" -match $dif})."ファイル数"
#$rdate=($newcsv|where-Object {$_."ID" -match $dif})."修改日期"
$cdate=($newcsv|where-Object {$_."ID" -match $dif})."作成日"
$name1=($newcsv|where-Object {$_."ID" -match $dif})."ファイル名"
...
if($web -match "1024549"){$till1="遷移元に戻る"}
if($web -match "1024480"){$till1="フォルダ詳細"}
...
$check_path=($check_path0.replace(" ","")).replace("URL情報をクリップボードにコピー","")|Out-String
...
#check download complete
 do{
 start-sleep -s 5
 $check_ongoings =(Get-ChildItem -Path "D:\Users\user30\Downloads\*.crdownload").count+(Get-ChildItem -Path "D:\Users\user30\Downloads\*.tmp").count
 $check_ongoings
...
if($goemon_folder  -match "フリセレ型番" -or $goemon_folder -match "スマート型番"  -or $goemon_folder -match "標準PC型番" -or $goemon_folder -match "スマセレ型番"){
$add_to[-1]."Allion_Path"= "auto move to型番folder"
Move-Item -path D:\Public\_AutoTask\RC\$folder2 -Destination  D:\Public\_AutoTask\RC\_型番一覧 -Force
}

if($goemon_folder  -match "型番データ"){
copy-Item -path D:\Public\_AutoTask\RC\$folder2 -Recurse -Destination  D:\Public\_AutoTask\RC\_型番一覧 -Force
}
    
   if($goemon_folder  -match "Manual査閲"){
$add_to[-1]."Allion_Path"= "auto move to Manual folder"
Move-Item -path D:\Public\_AutoTask\RC\$folder2 -Destination  D:\Public\_AutoTask\RC\_Manual相關 -Force

}
    
   
   if($goemon_folder  -match "イベントログ一覧" -or $goemon_folder  -match "OS関連\(won'tfix一覧"){
$add_to[-1]."Allion_Path"= "auto move to _Eventlog folder"
Move-Item -path D:\Public\_AutoTask\RC\$folder2 -Destination  D:\Public\_AutoTask\RC\_Eventlog -Force
