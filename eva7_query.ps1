...
$eva_path0="\\192.168.20.20\sto\EO\VD1\Dept-2\nec_tc\01.Driver_G\02.評價相關\01.評價依賴票\"
...
$file_names1=(gci -path $eva_path  -file  "*.xls*"| Where-Object {$_.fullname -notmatch "z-info" -and $_.fullname -notmatch "old"  -and $_.fullname -notmatch "Sample" -and $_.fullname -notmatch "Training" -and $_.fullname -notmatch "待確認" }).fullname
...
foreach ($dif in $file_names){
  ...
  $Folder=(Split-Path $eva_file_link).replace("$eva_path0","")
