...
$nowID= (get-process -name OUTLOOK -ea SilentlyContinue).Id
...
########extract goemon number from attachment#######>
 ...
 $goeID1=$goeID1| Sort-Object| Get-Unique
   ...
   $rls_files=gci -path "\\192.168.56.49\Public\_AutoTask\RC\$check_ID_folder\*" -Include *ReleaseNote.xls*
      ...
      $rls_file0=((((((((((((($rls_files.name).replace("TP","")).replace("TH","")).replace("TW","")).replace("SC","")).replace("SE","")).replace("_NHNP","")).replace("_NPNH","")).replace("_NPNW","")).replace("_NP","")).replace("_NH","")).replace("_SC","")).replace("_SE","")).replace("__","_")
...
$fdname=$null
 ...
 if($check_mail.length -gt 0 -and $mail_done -match $check_mail){
   ...
   if(-not(test-path $20path)){
   new-item -ItemType directory $20path -Force |Out-Null
      }
   ...
   $fdname=(gci -path  $20path -Directory).Name -match  "\.$rls_fod\b"
